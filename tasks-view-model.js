...
function audioPlotColor()
{
      plotColor = '#FFF803';
      return plotColor;
}
  function audioPlotType()
  {
        plotType = 'buffer';
        return plotType;
  }
  function audioPlotFill()
   {
        fill = true;
        return fill
   }
  function audioPlotMirror()
   {
        mirror = true;
        return mirror
   }
    function audioPlotBufferData()
    {
        bufferData = any;
        return bufferData
    }
 exports.audioPlotColor = audioPlotColor
exports.audioPlotType = audioPlotType
exports.audioPlotFill = audioPlotFill
exports.audioPlotMirror = audioPlotMirror
exports.audioPlotBufferData = audioPlotBufferData
    ...
    viewModel.insert = function() {
        Dialogs.prompt("Todo List Name", "").then(result => {
            database.execSQL("INSERT INTO lists (list_name) VALUES (?)", [result.text]).then(id => {
                ...
                //var path = file_system_1.path.join(.knownFolders.documents().path, result.text);
            }, error => {
...
function createTable(database)
{
    console.log("testing......")
    var audioFolder = file_system_1.knownFolders.currentApp().getFolder("audio");
//var recordingPath = audioFolder.getFile("recording." + this.platformExtension());
console.log("test")
    audioFolder.eachEntity(function (entities) {
    var viewModel = new Observable();
    viewModel.lists = new ObservableArray([]);
    console.log("Just started")
    console.log(entity.name)
                database.execSQL("INSERT INTO lists (list_name) VALUES (?)", [entity.name]).then(id => {
                    this.lists.push({id: id, list_name: entity.name});
                }, error => {
                    console.log("INSERT ERROR", error);
                });
    ...
    return true
        ...
        console.log("Entities are empty or not working")
