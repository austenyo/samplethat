...
var AudioDemo = (function (_super) {
    ...
    {
        var audioFolder = file_system_1.knownFolders.currentApp().getFolder(label.text);
    }
...
function setRandomColor() {
    $("#files").css("background-color", getRandomColor());
}
...
function createViewModel(database) {
    ...
    viewModel.insert = function() {
        ...
        /*Dialogs.prompt("Folder Color", "blue").then(result1 => {
            database.execSQL("INSERT INTO lists (backgroundColor) VALUES (?)", [result1.text]).then(id => {
            this.lists.push({id: id, list_name: result.text, backgroundColor: result1.text});
